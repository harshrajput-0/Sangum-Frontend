...
export function OAuthButton({
  ...
  return (
    <button
      ...
      disabled={isDisabled}
      className={cn(
        "flex h-[46px] w-[46px] flex-shrink-0 items-center justify-center rounded-full border border-[var(--border-strong)] bg-[var(--surface)] text-[var(--text-secondary)] [transition:background_var(--t-fast),border-color_var(--t-fast),color_var(--t-fast)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-glow-purple)] disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:h-[19px] [&_svg]:w-[19px]",
        !isDisabled && "hover:border-[var(--text)] hover:bg-[var(--surface-hover)] hover:text-[var(--text)]",
        className,
      )}
    >
